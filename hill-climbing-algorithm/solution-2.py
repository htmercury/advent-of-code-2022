...
def find_dest_and_format(hill_data):
...
def solution():
    ...
    dest = find_dest_and_format(hill_data)
    path_queue = []
    visited = []
    init_climber = Climber(dest[0], dest[1])
    path_queue.append(init_climber)
    visited.append(dest)

    # travel from dest to find shortest route to an 'a'
    while len(path_queue) != 0:
        curr_climber = path_queue.pop(0)
        curr_elevation = hill_data[curr_climber.y][curr_climber.x]

        if curr_elevation == get_num_elevation('a'):
