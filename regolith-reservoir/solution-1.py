...
def insert_path(path, rock_map, x_offset):
    prev_point = path[0]
...
def create_map(paths):
    ...
    for _ in range(max_height):
        rock_map_row = []
        for _ in range(width_length + PADDING*2):
            rock_map_row.append('.')
        rock_map.append(rock_map_row)

    for p in paths:
        insert_path(p, rock_map, min_width)
...
def is_out_of_bounds(x, rock_map):
    x_length = len(rock_map[0])
    return x == 0 or x == x_length - 1
...
def drop_sand(sand_start, rock_map, moves):
    ...
    while True:
        ...
        if not moved:
            ...
            return is_out_of_bounds(sand_x, rock_map)
